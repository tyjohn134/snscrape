...
class _TwitterAPIScraper(snscrape.base.Scraper):
	...
	def _make_tweet(self, tweet, user, retweetedTweet = None, quotedTweet = None, card = None, noteTweet = None, **kwargs):
		...
		if tweet['entities'].get('user_mentions'):
			kwargs['mentionedUsers'] = [User(username = u['screen_name'], indices=u['indices'], id = u['id'] if 'id' in u else int(u['id_str']), displayname = u['name']) for u in tweet['entities']['user_mentions']]
		...
		if entities.get('hashtags'):
			kwargs['hashtags'] = [{'text':o['text'], 'indices':o['indices']} for o in entities['hashtags']]
